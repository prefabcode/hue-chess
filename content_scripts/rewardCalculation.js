...
export const getInitialRewardValue = (game) => {
	return new Promise((resolve) => {
		...
		let gameType;
		let rewardMultiplier = Math.ceil(
			3.5 * (estimatedDuration / bulletDuration),
		);
...
export const incrementHue = async (game) => {
	...
	try {
		...
		console.log(
			`Current hue value: ${currentValue}, New hue value: ${newValue}`,
		);
		...
		if (newValue >= 100) {
			...
			if (completedBoards >= 17) {
				prestige += 1;
				console.log(
					`Prestige level increased to: ${prestige}, and resetting to level 1`,
				);
			...
			await new Promise((resolve) => {
				browser.storage.local.set(
					{ completedBoards, currentHue: carryOverValue },
					resolve,
				);
			});
...
export const applyGladiatorPenalty = async () => {
	waitForElm("#user_tag").then((userTag) => {
		...
		waitForElm(".subs").then((subsDiv) => {
			console.log("Subs div detected");
			const boardButton = Array.from(subsDiv.querySelectorAll("button")).find(
				(button) => button.textContent === "Board",
			);
			if (!boardButton) {
			...
			waitForElm(".board-hue").then((boardHueDiv) => {
				...
				hueSlider.dispatchEvent(new Event("input"));
				console.log(
					`Applying gladiator penalty. Previous hue value: ${currentHue}, New hue value: ${newHue}`,
				);
				userTag.click();
			});
		});
