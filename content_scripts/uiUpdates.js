...
export const updateProgressBar = (completedBoards = null, hueValue = null) => {
	browser.storage.local.get(["completedBoards"], (result) => {
		const level =
			(completedBoards !== null ? completedBoards : result.completedBoards) + 1;
		...
		const isDarkMode =
			...
			document.body.classList.contains("transp");
		if (isDarkMode) {
...
async function setImageSources() {
	...
	images.forEach((imageId) => {
		...
		browser.storage.local.get(["completedBoards"], (result) => {
			const playerLevel =
				(result.completedBoards !== null ? result.completedBoards : 0) + 1;
			if (playerLevel >= unlockLevel) {
				imageElement.src = browser.runtime.getURL(
					`imgs/${imageId.replace("-icon", "")}.svg`,
				);
			} else {
...
export const openPerksModal = async () => {
	try {
		...
		document
			.getElementById("close-perks-modal")
				...
				if (perk === "gladiator") {
					if (!isActive) {
						const confirmSelection = confirm(
							"Warning: You will not be able to remove the Gladiator perk until you level up or suffer the hue point penalty. Do you want to proceed?",
						);
						if (!confirmSelection) {
							return;
						} else {
						...
						if (!canRemove) {
							alert(
								"You cannot remove the Gladiator perk until you level up or suffer the hue point penalty.",
							);
							return;
						}
					}
...
export const updatePerksModalContent = async () => {
	browser.storage.local.get(
		["completedBoards", "currentHue", "activePerks", "prestige"],
		(result) => {
			const playerLevel =
				(result.completedBoards !== null ? result.completedBoards : 0) + 1;
			...
			if (prestige) {
				...
				let existingPrestigeIcon =
					prestigeContainer.querySelector(".prestige-icon");
				...
				const imageFormat = [1, 10, 15, 16].includes(imageNumber)
					? "svg"
					: "jpg";
				const imagePath = browser.runtime.getURL(
					`imgs/prestige/${imageNumber}.${imageFormat}`,
				);
			...
			perkBoxes.forEach((box) => {
				...
				if (playerLevel >= unlockLevel) {
					...
					box.setAttribute(
						"data-tippy-content",
						box.getAttribute("data-tippy-content-original"),
					);
				} else {
					box.style.display = "flex";
...
const openSettingsModal = async () => {
	try {
		...
		document
			...
			.addEventListener("click", confirmResetProgress);
		document
			.getElementById("sync-ui")
			.addEventListener("click", syncLichessUIWithExtensionState);
		...
		if (process.env.NODE_ENV !== "production") {
			...
			document.getElementById("export-state").addEventListener("click", () => {
				browser.storage.local.get(
					["initialized", "completedBoards", "currentHue", "prestige"],
					(result) => {
	...
	waitForElm("#user_tag").then((userTag) => {
		...
		waitForElm(".subs").then((subsDiv) => {
			...
			waitForElm(".list").then((boardList) => {
				...
				const targetBoardButton = boardList.querySelector(
					`button[title="${targetBoardTitle}"]`,
				);
	...
	browser.storage.local.get(
		...
		async (result) => {
			...
			if (activePerks.length === 0) {
				tooltipContent =
					"<p>No perks selected. Click the progress bar to select perks!</p>";
			} else {
				...
				if (activePerks.includes("versatility") && playedOpenings.length) {
					tooltipContent +=
						'<br><p>Played Openings:</p><ul class="versatility-openings">';
...
export const resetUserMenuState = async () => {
	try {
		...
		if (backButton) {
			backButton.click();
			console.log(
				"Menu was not in default state, clicked back button to reset",
			);
		} else {
		...
		console.error(
			"An error occurred while resetting the user menu state:",
			error,
		);
	}
};
...
export const createChallengeCompletionModal = () => {
  ...
  `;
	...
	document
		.getElementById("close-hue-challenge-completion-modal")
		.addEventListener("click", () => {
			dialog.close();
			dialog.remove();
		});
	...
	document
		.getElementById("close-hue-challenge-completion-modal-x")
		.addEventListener("click", () => {
			dialog.close();
			dialog.remove();
		});
