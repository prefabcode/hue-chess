...
export function showPerkToast(perkId, message) {
	...
	const imageUrl =
		perkId !== "total-earned"
			? browser.runtime.getURL(`imgs/${perkId}.svg`)
			: "";
...
const isBerzerkerFulfilled = (userName, game) => {
	...
	console.log(
		`Initial Time: ${initialTime} seconds, Increment: ${increment || 0} seconds`,
	);
	...
	console.log(
		`Last Move Remaining Time for ${playerColor}: ${remainingTimeInSeconds} seconds`,
	);
...
const isGambiteerFulfilled = (userName, game) => {
	...
	for (const move of moves) {
		if (
			move.notation.notation === "O-O-O" &&
			move.turn === playerColor.charAt(0)
		) {
			const bonus = calculateRandomBonus(3, 5);
			console.log(`Gambiteer bonus points: ${bonus}`);
			const message = `Gambiteer: ${bonus} points`;
...
const isEndgameSpecialistFulfilled = (game) => {
	if (game.tags.Variant !== "Standard" && game.tags.Variant !== "Chess960") {
		console.log(
			"Standard or Chess960 not detected. Endgame Specialist disabled.",
		);
		return 0;
	}
	const moves = game.moves;
...
const isHueFocusFulfilled = () => {
	...
	if (hasNoRatingClass) {
		...
		console.log(
			`body has no-rating class, adding ${bonus} hue points to bonus`,
		);
		return bonus;
	}
	return 0;
...
const calculateMaterialBalanceFromFEN = (fen, playerColor) => {
	...
	return playerColor === "white"
		? pieceCount.white - pieceCount.black
		: pieceCount.black - pieceCount.white;
};
